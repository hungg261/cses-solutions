...
Code: 3226
Time (YYYY-MM-DD-hh.mm.ss): 2025-09-29-08.56.26
*******************************************************************************/
#include<bits/stdc++.h>
...
struct SegmentTree{
    struct Node{
        ...
        static Node Identity(){
            Node res;
            res.Sum = res.maxPre = res.maxSuf = res.maxSum = 0;
    ...
    void build(int id, int l, int r){
        if(l == r){
            nodes[id].Sum = arr[l];
            nodes[id].maxPre = nodes[id].maxSuf = nodes[id].maxSum = max(0LL, arr[l]);
            return;
        }
    ...
    void update(int id, int l, int r, int idx, int val){
        ...
        if(l == r){
            nodes[id].Sum = val;
            nodes[id].maxPre = nodes[id].maxSuf = nodes[id].maxSum = max(0LL, val);
            return;
        }
